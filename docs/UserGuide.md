...
- `p/PHONE` (compulsory): phone number
- `e/EMAIL` (compulsory): email address
- `a/ADDRESS` (compulsory): address
- `s/STAGE` (compulsory): investigation stage
- `al/ALIAS` (optional): one or more aliases (**comma-separated**)
- `r/RISK` (optional): risk level; default is `medium`
- `note/NOTES` (optional): initial notes (up to 500 characters)
- `pw/PASSWORD` (optional): contact password
- `t/TAG` (optional): repeatable tags

**Examples**
- `add n/John Tan p/98765432 e/johntan@example.com a/Clementi Ave 2 s/surveillance`
- `add n/Michael Lee p/91234567 e/mike@example.com a/Marina Bay s/approached al/Big Mike r/high note/Seen at Marina Bay`
...
---|---|---
Add Contact | `add n/NAME p/PHONE e/EMAIL a/ADDRESS s/STAGE [al/ALIAS(,ALIAS...)] [note/NOTES] [r/RISK] [pw/PASSWORD] [t/TAG]...` | `add n/John Tan p/98765432 e/john@example.com a/Clementi Ave 2 s/surveillance`
